...
export default function RentManagementPage() {
    ...
    return (
       <div className="min-h-screen bg-white animate-in fade-in duration-500 font-body pb-32">
           <header className="px-4 py-4 flex items-center gap-6 sticky top-0 bg-white z-50 border-b border-slate-50">
              <button onClick={() => router.push('/rent')} className="text-[#00685F] active:scale-90 transition-transform">
              ...
              <h1 className="text-xl font-black text-[#1A2B28]">Financial Analytics</h1>
           </header>
           <main className="px-6 py-8 space-y-8">
              <div className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-50 space-y-6">
                 ...
                 <div className="flex justify-between text-[10px] font-bold text-[#718096] uppercase px-1"><span>Oct</span><span>Nov</span><span>Dec</span><span>Jan</span><span>Feb</span><span>Mar</span></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div className="bg-white p-5 rounded-2xl border-l-4 border-[#008075] shadow-sm"><p className="text-[12px] font-bold text-[#718096] mb-1">Target</p><p className="text-xl font-black text-[#1A2B28]">₹1.24L</p></div>
                 <div className="bg-white p-5 rounded-2xl border-l-4 border-amber-500 shadow-sm"><p className="text-[12px] font-bold text-[#718096] mb-1">Achieved</p><p className="text-xl font-black text-[#1A2B28]">₹1.08L</p></div>
              </div>
           </main>
        </div>
    )
  ...
  if (view === "paid" || view === "unpaid") {
    ...
    return (
       <div className="min-h-screen bg-white animate-in fade-in duration-500 font-body pb-32">
           <header className={`px-4 py-4 flex items-center gap-6 sticky top-0 z-50 bg-white border-b border-slate-50`}>
              <button onClick={() => router.push('/rent')} className="text-[#00685F] active:scale-90 transition-transform">
                 <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
              </button>
              <div className="flex flex-col">
                 <h1 className="text-xl font-black text-[#1A2B28]">{view === 'paid' ? 'Collection History' : 'Outstanding Rent'}</h1>
                 <span className={`text-[12px] font-bold mt-0.5 text-[#008075]`}>{list.length} Total Entries</span>
              </div>
           </header>

           <main className="px-6 py-8 space-y-8">
              <div className="space-y-2">
                 <h2 className="text-[22px] font-bold text-[#1A2B28]">{view === 'paid' ? 'Transaction Ledger' : 'Arrears Directory'}</h2>
              ...
              <div className="space-y-4">
                 <h3 className="text-[20px] font-bold text-[#1A2B28]">{view === 'paid' ? 'Paid Transactions' : 'Pending Payments'} ({list.length})</h3>
              ...
              <div className="relative bg-white w-full max-w-lg rounded-t-[40px] px-6 pt-10 pb-12 animate-in slide-in-from-bottom duration-500 shadow-[0_-15px_50px_-15px_rgba(0,0,0,0.3)]">
                 ...
                 <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-y-8 pt-2">
                       <div className="space-y-1"><p className="text-[12px] font-bold text-[#718096]">Tenant</p><p className="text-[16px] font-bold text-[#1A2B28]">{selectedTenant?.name}</p></div>
                       <div className="space-y-1"><p className="text-[12px] font-bold text-[#718096]">Room</p><p className="text-[16px] font-bold text-[#1A2B28]">{selectedTenant?.room}</p></div>
                       <div className="space-y-1 bg-[#F1F4F8] p-4 rounded-2xl border-l-4 border-[#00685F]">
                          <p className="text-[10px] font-bold text-[#718096] uppercase tracking-widest mb-1 ml-1">Rent Amount</p>
                       ...
                       <div className="space-y-1 bg-[#F1F4F8] p-4 rounded-2xl border-l-4 border-amber-500">
                          <p className="text-[10px] font-bold text-[#718096] uppercase tracking-widest mb-1 ml-1">Payment Date</p>
                    ...
                    <div className="space-y-3">
                       <p className="text-[10px] font-bold text-[#718096] uppercase tracking-widest ml-1">Payment Mode</p>
        ...
        <div className="grid gap-5 mt-10">
          ...
          >
            ...
            <div className="flex flex-col items-start text-left">
              ...
              <span className="text-[12px] font-bold text-red-500 mt-1">{unpaidTenants.length} Pending Collections</span>
            </div>
          </button>
          ...
          >
            ...
            <div className="flex flex-col items-start text-left">
              ...
              <span className="text-[12px] font-bold text-[#008075] mt-1">{paidTenants.length} Cleared this Month</span>
            </div>
          </button>
