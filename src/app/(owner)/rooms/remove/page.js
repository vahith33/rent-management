  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(result.error || "Failed to remove room");
      }
    } catch (err) {
          ...
          {isLoading ? (
            ...
            <div key={room.id} className="bg-white p-5 rounded-[32px] border border-slate-100 shadow-sm flex items-center justify-between group transition-all hover:border-red-100 hover:shadow-lg">
              <div className="flex items-center gap-4">
                 <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm transition-colors ${room.status === 'OCCUPIED' ? 'bg-slate-100 text-slate-400' : 'bg-red-50 text-red-500'}`}>
                 ...
                 <div className="flex flex-col">
                    ...
                    <p className={`text-[12px] font-bold mt-0.5 ${room.status === 'OCCUPIED' ? 'text-red-400' : 'text-[#718096]'}`}>
