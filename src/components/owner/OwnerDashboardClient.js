    ...
    <div className="min-h-screen pb-24 font-sans">
      <main className="px-6 pt-5 space-y-8">
        ...
        <div className="grid grid-cols-2 gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
          ...
          <div onClick={() => router.push('/rooms?view=list')} className="bg-white p-4 rounded-[24px] shadow-sm cursor-pointer active:scale-95 transition-all">
            <span className="text-[11px] font-bold text-[#ADB5BD] uppercase tracking-wider">Vacant Beds</span>
            <div className="mt-1 space-y-2">
              <span className="text-2xl font-black text-[#1A2B28]">{stats.vacantBeds}</span>
          ...
          <div onClick={() => router.push('/rent?view=unpaid')} className="bg-white p-4 rounded-[24px] shadow-sm cursor-pointer active:scale-95 transition-all">
            <span className="text-[11px] font-bold text-[#ADB5BD] ">Pending Rent</span>
            <div className="mt-1">
              <span className="text-2xl font-black text-red-500">{stats.pendingRentCount}</span>
            </div>
          </div>

          <div onClick={() => router.push('/rent?view=analytics')} className="bg-white p-4 rounded-[24px] shadow-sm cursor-pointer active:scale-95 transition-all">
            <span className="text-[11px] font-bold text-[#ADB5BD] ">Monthly Income</span>
            <div className="mt-1">
              <span className="text-2xl font-black text-[#1A2B28]">₹{stats.monthlyIncome}</span>
            </div>
          </div>
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-300">
          ...
          <div className="space-y-3">
            {recentRent.filter(t => t.status === rentFilter).map((rent, i) => {
              const isPartial = rent.status === 'UNPAID' && rent.tenants?.rent && Number(rent.amount) < Number(rent.tenants.rent);
              const displayStatus = isPartial ? 'PARTIAL' : rent.status;
              const statusColorClass = rent.status === 'PAID' 
                ...
                <div key={i} className="bg-white p-4 rounded-[28px] shadow-sm border border-slate-50 flex items-center justify-between hover:shadow-md transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[13px] font-medium text-[#ADB5BD]">Due: {new Date(rent.due_date).toLocaleDateString('en-GB')} • ₹{rent.amount}</span>
                    </div>
                  </div>
                  <div className={`px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest ${statusColorClass}`}>
                    {displayStatus}
                  </div>
                </div>
              );
